...
def generate_laplacien_matrix(n):
    N = n*n
...
# Given a n*n image, compute the corresponding vector v
# Then compute the eigenvalues of  (v * generate_laplacien_matrix(n))
def compute_eigenvalues(img):
    n = img.shape[0]
    img.resize(n*n,1)
    M = generate_laplacien_matrix(n)
    laplacien = M * img
    w, vectors = linalg.eig(laplacien)
    return sorted(-w)
